...
1. Nas aulas dessa seção, vimos que somente com a instrução SELECT, é possivel realizarmos operações matemáticas simples
e em alguns casos, e com o auxilio de outros comandos, algo mais complexo.

O seu desafio é: Com base nos 6 números apresentados abaixo, desenvolva uma instrução SELECT que apresente a média
aritmética conforme resultado abaixo:
...
2. Em certos processos de negócios, precisamos capturar o endereço de clientes e fornecedores para compor documentos
como nota fiscal, por exemplo.

Esse desafio consiste em você concatenar as informações separadas de um endereço contidas entre aspas simples abaixo e
apresentar em uma única informação conforme o resultado esperado.
...
Obs: Se você conseguir apresentar os dados de endereço, sem alterar os conteúdos dos valores originais entre aspas
simples, você já apresenta um bom entendimento de concatenação.
*/
SELECT
    'Rua' + ' ' + 'Conselheiro Furtado' + ', ' + 'No. 897' + ', ' + 'Ap 50 - Bloco A' + ' - ' + 'Santa Efigênia' + ' - ' + 'Capital' + ' ' + 'São Paulo' + '- ' + 'Cep: 01001-001' AS Endereco
GO
...
3. Dados de fornecedores são importantes para a área de compras de um empresa, para realizar os pedidos de produtos.
Monte uma instrução SELECT a partir da tabela FORNECEDOR e recupere os dados que permitem realizar a comunicação com os
fornecedores. Veja o resultado esperado.
...
(29 rows affected)
Obs: Os três pontos no meio do resultado significa que existem mais linhas entre as 3 linhas iniciais e as 3 linhas
...
/*
4. Agora a empresa deseja enviar formalmente para os fornecedores, os modelos de produtos que eles devem entregar. Para
isso a empresa deve enviar somente para os contatos dos fornecedores com o cargo Sales Representative (Representante de
vendas). Para realizar esse envio, a empresa precisa de uma relação com o nome do contato e o endereço completo,
conforme apresentado no resultado abaixo.
...
/*
5. A empresa mantém contatos com vários fornecedores. Principalmente com os fornecedores do velho continente. Agora você
precisa gerar um relatório para reunião da diretoria, a relação de fornecedores da Germany e France (Alemanha e França).
A apresentação do relatório deve ser igual ao resultado abaixo.
...
/*
6. A área de compras da empresa precisa saber a relação de Produtos do fornecedor Supplier VHQZD. Você deve montar um
...
SELECT
    NomeProduto
  , PrecoUnitario
  , PrecoUnitario * 0.9 AS PrecoDesconto,iIDFornecedor
FROM
    Producao.Produto
WHERE
    iIDFornecedor = (SELECT iIDFornecedor FROM Producao.Fornecedor WHERE RazaoSocial = 'Supplier VHQZD')
    AND iIDFornecedor = 2
GO

/*
7. Agora a empresa precisa cadastrar um novo Fornecedor. E para esse fornecedor, você terá que cadastrar um novo Produto.
Abaixo segue os dados do Fornecedor:
...
Você precisa criar os comandos de INSERT para cada um dos conjuntos de dados acima e inseri-lós na tabelas FORNECEDOR e
...
/*
9. Por fim, a empresa decidiu excluir o fornecedor e o produto. Você terá que criar os comandos para excluir o
Fornecedor e o Produto.
*/
DELETE FROM
    Producao.Produto
WHERE
    iIDFornecedor = (SELECT iIDFornecedor FROM Producao.Fornecedor WHERE RazaoSocial = 'Supplier ABCDE')
AND
	iIDFornecedor = 30
GO

/*10. Agora vamos para um desafio. Vamos utilizar a tabela PEDIDO que está no esquema VENDAS. Um desenvolvedor criou um
relatório a partir da consulta abaixo e com o seguintes resultado:
...
(830 rows affected)
Mas a diretoria precisa somente da relação dos pedidos realizados no ano de 2007. Voce deverá realizar a alteração na
